...
some day using some device.
 
We define the install date of a player to be the first login day of that player.

We also define day 1 retention of some date X to be the number of players whose install date is X and they logged
...
SELECT
    ...
    COUNT(DISTINCT t.player_id) AS 'installs',
    ROUND(SUM(CASE WHEN a.event_date IS NULL THEN 0 ELSE 1 END)/COUNT(DISTINCT t.player_id), 2) AS 'Day1_retention'
FROM
(
    SELECT 
        player_id, 
